...
def ue_slice_type_explainer(sim, knowledge_router, query_key, params):
    lines = [
        ...
        f"    corresponding QoS profile: {json.dumps(NETWORK_SLICES[NETWORK_SLICE_EMBB_NAME])}",
        ...
        f"    corresponding QoS profile: {json.dumps(NETWORK_SLICES[NETWORK_SLICE_URLLC_NAME])}",
    ]
    ...
    lines.append("\nWhen the UE picks up a slice type, it will also get the corresponding QoS profile (query /user_equipments/attribute/qos_profile for more details).\n")
    return "\n".join(lines)
